...
f = open("_OurMethod_Time.txt", "w")

RECORDS_IN_CHUNK = 155  # 1000 lub 2300
#NUMBER_OF_CHUNK = 1000

# Load data and split data into chunk.
#input_dir_path = 'Arff_new/'
...
#print("Number of chunks = ", len(dataSplit))
f.write("Number of chunks = %d\n" % (len(dataSplit)))

(feat0, threshold) = computeSTDinChunk(dataSplit[0], RECORDS_IN_CHUNK / 10)
...
for i in range(1, numberOfChunk):
    ...
    if pos[0][0] > threshold:
        ...
        if (i + 1) < numberOfChunk:
            (feat0, threshold) = computeSTDinChunk(dataSplit[i + 1], RECORDS_IN_CHUNK / 10)

print("Accuracy", np.mean(results))
f.write("Our method time = %2.2f \n" % (sum_elapse_time))
print("Our method time = %2.2f \n" % (sum_elapse_time))
f.close()
...
for i in range(0, len(drifts)):
    drifts[i] *= RECORDS_IN_CHUNK

print("Nasza metoda = ", drifts)
