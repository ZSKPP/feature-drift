...
debug=1
(feat0, threshold) = computeSTDinChunk(dataSplit[0], RECORDS_IN_CHUNK / 10, debug)
...
for i in range(1, numberOfChunk):
    ...
    if pos[0][0] > threshold:
        ...
        if chunk < numberOfChunk:
            ...
            forest = RandomForestClassifier(random_state = 0)
            forest.fit(X_train, y_train)
            (feat0, threshold) = computeSTDinChunk(dataSplit[chunk], RECORDS_IN_CHUNK / 10, debug)
et = time.time()
elapsed_time = et - st

print(len(drifts))
#plt.figure().set_figheight(2)
print(round(np.mean(results),5))
if len(drifts) > 0:
...
plt.grid(True)
plt.savefig("Figure_" + str(RECORDS_IN_CHUNK) + ".pdf")
plt.show()


for i in range(0, len(drifts)):
    drifts[i] *= RECORDS_IN_CHUNK
print("Nasz metoda = ", len(drifts))
